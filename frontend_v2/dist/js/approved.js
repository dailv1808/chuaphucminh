document.addEventListener('alpine:init', () => {
  Alpine.data('approvedData', () => ({
    ...
    async confirmCheckIn() {
      ...
      try {
        const assignResponse = await fetch('${window.API_BASE_URL}/api/kutiassignment/assign/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        const token = localStorage.getItem('access_token');
        const checkInResponse = await fetch(`${window.API_BASE_URL}/api/registration/${this.currentRegistrationId}/`, {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    async downloadTemporaryStay(registration) {
      try {
        ...
        const token = localStorage.getItem('access_token'); 
        const response = await fetch('${window.API_BASE_URL}/api/tamtru/', {
          method: 'POST',
          headers: {
     	    'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
