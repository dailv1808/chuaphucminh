  ...
  Alpine.data('slideshowData', function() {
    return {
      ...
      createWelcomePDFSlide: function(doc) {
        ...
        const lines = ['HỎI PHÁP', 'TRÌNH PHÁP'];
        let y = 80;
      ...
      createQuestionPDFSlide: function(doc, question, index) {
        // Reset background
        ...
        doc.setFontSize(20);
        doc.setFont('Times', 'bold');
        doc.text(`Câu hỏi ${index + 1}`, 15, 20);
        ...
        // Xử lý từng dòng với định dạng phù hợp
        const lines = content.split('\n');
        
        lines.forEach(line => {
          if (line.trim() === '') {
            y += 4; // Khoảng trắng giữa các đoạn
            return;
          }
          ...
          y += 2; // Thêm khoảng cách sau mỗi đoạn
        });
