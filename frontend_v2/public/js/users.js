  ...
  Alpine.data('usersData', () => ({
    ...
    async fetchUsers() {
      try {
        ...
        const response = await fetch('http://192.168.0.200/api/accounts/users/', {
          headers: {
    ...
    async saveUser() {
      try {
        ...
        if (this.selectedUser) {
          ...
          const url = `http://192.168.0.200/api/accounts/profile/`;
          ...
          const url = 'http://192.168.0.200/api/accounts/register/';
          response = await fetch(url, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
